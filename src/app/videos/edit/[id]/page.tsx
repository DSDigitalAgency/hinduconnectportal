  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const item = data.item || data; // API returns { item }
        if (!res.ok || !item) {
  ...
  if (fetching) {
    return (
      <div className="p-4 sm:p-8 bg-[#fff7ed] min-h-screen">
        <div className="max-w-3xl mx-auto text-center py-12">Loading video...</div>
      </div>
    );
  }
    ...
    <div className="p-4 sm:p-8 bg-[#fff7ed] min-h-screen">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-3xl font-extrabold text-orange-600 tracking-tight">Edit Video</h1>
          <button
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-end space-x-3">
              ...
              <button type="submit" disabled={saving} className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-50">{saving ? 'Updating...' : 'Update Video'}</button>
            </div>
          </form>
        </div>
      </div>
